...
for (const t of TARGETS) {
  const name = `@vcode/cli-${t.node}`;
  const dir = join(STAGE, `cli-${t.node}`);
  const exe = t.goos === "windows" ? "vcode.exe" : "vcode";
    ...
    `${JSON.stringify(
      {
        name,
        version,
        description: `vcode prebuilt binary for ${t.node}.`,
        ...
        repository: {
          type: "git",
          url: "git+https://github.com/esengine/DeepSeek-vcode.git",
        },
      },
      null,
      2,
...
const mainDir = join(STAGE, "vcode");
mkdirSync(mainDir, { recursive: true });
cpSync(join(HERE, "vcode", "bin"), join(mainDir, "bin"), { recursive: true });
cpSync(join(ROOT, "README.md"), join(mainDir, "README.md"));

const mainPkg = JSON.parse(
  readFileSync(join(HERE, "vcode", "package.json"), "utf8"),
);
...
// stable to default with a manual `npm dist-tag add vcode@<ver> latest`.
...
console.log(`publish vcode@${version} (${distTag})`);
