...
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "vcode-installsource-test-*")
	if err != nil {
...
func TestApplyLocalSkillRootRegistersPath(t *testing.T) {
	...
	cfg := config.LoadForEdit(filepath.Join(project, "vcode.toml"))
...
func TestApplyLocalCodexPluginPackage(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	statePath := filepath.Join(home, ".vcode", "plugin-packages.json")
	...
	if _, err := os.Stat(filepath.Join(home, ".vcode", "plugins", "superpowers", ".codex-plugin", "plugin.json")); err != nil {
...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".vcode", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	existing := filepath.Join(project, ".vcode", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".vcode", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".vcode", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".vcode", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".vcode", "skills", "gamma", "SKILL.md")
...
func TestPlanProjectMCPJSONDefaultsProject(t *testing.T) {
  ...
  "mcpServers": {
	...
	wantPath := filepath.Join(project, "vcode.toml")
...
func TestApplyRemoteMCPURLDefaultsGlobal(t *testing.T) {
	...
	projectCfg := config.LoadForEdit(filepath.Join(project, "vcode.toml"))
...
func TestApplyMCPRejectsDuplicateByDefault(t *testing.T) {
	...
	if err := cfg.SaveTo(filepath.Join(project, "vcode.toml")); err != nil {
		t.Fatal(err)
	}
...
func TestApplyMCPReplaceOverwritesExisting(t *testing.T) {
	...
	reloaded := config.LoadForEdit(filepath.Join(project, "vcode.toml"))
...
func TestApplyMCPRollsBackOnSaveFailure(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(project, "vcode.toml"), 0o755); err != nil {
		t.Fatal(err)
	}
	writeFile(t, filepath.Join(project, "vcode.toml", "blocker"), "x")
...
func TestGlobalSkillInstallRootUsesVcodeHome(t *testing.T) {
	home := t.TempDir()
	...
	want := filepath.Join(vcodeHome, skill.SkillsDirname)
	if root != want {
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	target := filepath.Join(project, ".vcode", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	projectTarget := filepath.Join(project, ".vcode", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".vcode", "skills", "dupe.md")
...
func TestPlanIDIncludesActionDetails(t *testing.T) {
	...
	a := action{Kind: "mcp", Action: "install_mcp_server", Name: "same", URL: "https://mcp.one.example/mcp", Transport: "http", ConfigPath: "/repo/vcode.toml"}
	b := a
