...
func TestMain(m *testing.M) {
	...
	// the environment (VCODE_LANG/LC_ALL/LC_MESSAGES/LANG) and installs it as
...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"vcode", "› /mcp"}
	...
	if !strings.Contains(lines[0], "vcode") || !strings.Contains(lines[1], "/mcp") {
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "vcode.toml")
