...
type chatTUI struct {
	...
	// "/mouse" or VCODE_DISABLE_MOUSE at startup; trades away in-app
	// drag-select, the transcript scrollbar, and wheel-scroll while it's on,
	// since the terminal no longer forwards those events to Vcode.
	mouseCaptureOff bool

	shimmerFrame int // frame counter for the golden Vcode… shimmer animation
	...
	// planMode mirrors the agent's read-only gate (Tab toggles it). The
	// marker rides in outgoing user messages so the cache-stable prompt prefix is
	// left untouched.
	planMode bool
	// goalMode is set when Tab cycles to goal mode. In goal mode the user types
	// a goal and sends it; the controller enters goal-driven mode.
	goalMode bool
	// tokenMode tracks the current token saver mode. "" or "full" = full profile,
	// "economy" = lean profile. Shift+Tab toggles it via async controller rebuild.
	tokenMode string
	...
	// commands are custom slash commands loaded from .vcode/commands; each renders
...
func mouseCaptureOffByDefault() bool {
	v := strings.TrimSpace(os.Getenv("VCODE_DISABLE_MOUSE"))
	return v != "" && v != "0"
}
...
func (m chatTUI) update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tokenModeSwitchMsg:
		if msg.err != nil {
			m.notice("token mode: " + msg.err.Error())
		} else {
			m.ctrl = msg.ctrl
			m.label = msg.label
			m.commands = msg.commands
			m.skills = msg.skills
			m.host = msg.host
			m.tokenMode = msg.mode
			if msg.oldCtrl != nil {
				m.oldControllers = append(m.oldControllers, msg.oldCtrl)
			}
			if msg.mode == "economy" {
				m.notice("token mode: 省 token")
			} else {
				m.notice("token mode: 完整 token")
			}
			cmds = append(cmds, fetchBalance(m.ctrl))
			if c := m.runStatusline(); c != nil {
				cmds = append(cmds, c)
...
// "cache: turn 88.00% · total 78.00%": the single-turn rate (latest turn, the higher/steeper
...
// tokenTag returns a status tag showing the current token mode.
func (m chatTUI) tokenTag() string {
	switch m.tokenMode {
	case "economy":
		return lipgloss.NewStyle().Foreground(lipgloss.Color("#10b981")).Bold(true).Render("省")
	case "full":
		return lipgloss.NewStyle().Foreground(lipgloss.Color("#6366f1")).Bold(true).Render("完整")
	}
	return ""
}
...
// toggleMouseCapture flips whether Vcode owns the mouse. It's session-only
...
func (m *chatTUI) startTurnWithRaw(sent, displayed, restore, raw string) tea.Cmd {
	...
	return tea.Batch(m.spinner.Tick, elapsedTick(), shimmerTickCmd())
}
...
func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	b.WriteString(themeFg(vcodeBrandGold, "◆") + " " + bold("vcode") + "  " + dim("· "+label) + "\n")
