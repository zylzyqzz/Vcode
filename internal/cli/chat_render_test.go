...
func TestCacheRateLabelKeepsTwoDecimals(t *testing.T) {
	if got := cacheRateLabel("cache: turn %s", 998, 1000); got != "cache: turn 99.80%" {
		t.Fatalf("cacheRateLabel = %q, want cache: turn 99.80%%", got)
	}
	if got := cacheRateLabel("total %s", 1, 3); got != "total 33.33%" {
		t.Fatalf("cacheRateLabel = %q, want total 33.33%%", got)
	}
	if got := cacheRateLabel("total %s", 1, 0); got != "" {
