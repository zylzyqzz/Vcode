...
// mcp.go holds the MCP server-management surface shared by the `vcode mcp`
...
// mcpCommand implements `vcode mcp <add|remove|list>`. It edits config only
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: vcode mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to vcode.toml).

Usage:
  vcode mcp list
  vcode mcp add <name> <command> [args...]        stdio server
...
Examples:
  vcode mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  vcode mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
