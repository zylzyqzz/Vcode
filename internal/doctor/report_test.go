...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("VCODE_TEST_SECRET", "sk-live-secret")
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	t.Setenv("VCODE_HOME", filepath.Join(t.TempDir(), "vcode"))
	...
	if !strings.Contains(text, "vcode 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config vcode.toml: parse boom"}})
