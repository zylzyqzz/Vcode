// Package memory implements Vcode's persistent memory. It mirrors Claude
// Code's two-layer model while honoring Vcode's cache-first architecture:
//
//   - Hierarchical doc memory: VCODE.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.vcode/VCODE.md
	ScopeAncestor Scope = "ancestor" // a VCODE.md above the project root
	ScopeProject  Scope = "project"  // ./VCODE.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./VCODE.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// VCODE.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"VCODE.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"VCODE.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
