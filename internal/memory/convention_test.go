...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing VCODE.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "VCODE.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "VCODE.md" {
		t.Errorf("should append to the existing VCODE.md, got %s", got)
	}
