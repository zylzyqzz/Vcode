...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — vcode setup
	StepSetKey      string // step 2 command label

	// `vcode init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // interactive session step desc
	StepRunDesc    string // vcode run step desc
	HelpFooter     string // dim footer linking to vcode help
	...
	WroteFileFmt          string // "Wrote %s" — used for vcode.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from vcode.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: vcode run [--model NAME] <task>"
	...
	// `vcode upgrade` / `vcode update` — self-update
...
//
// Priority: override > VCODE_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"VCODE_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
