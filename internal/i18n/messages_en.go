...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `vcode`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold vcode.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `vcode`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `vcode setup`.",
	StepSetKeyHint: "run `vcode setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "vcode help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `vcode`",
	...
	OutputStyleHint:    "set agent.output_style in vcode.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in vcode.toml",
	...
	ArgLanguageAuto:     "auto-detect from VCODE_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .vcode/settings.json (project, after trust) or ~/.vcode/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in vcode.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create VCODE.md in the project root",
	...
	MissingKeyIntro:       "vcode.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `vcode setup` or export DEEPSEEK_API_KEY=...), then run `vcode run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from vcode.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: vcode run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `vcode setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `vcode setup`.",
	...
	UsageBody: `vcode — a config- and plugin-driven coding agent (multi-model)
...
Usage:
 ...
 vcode task list|global|create|show|logs|events|agents inspect durable task graph and Agent state
  ...
  vcode upgrade [--check] [--force]                   self-update to the latest release (also: vcode update)
  vcode version
  vcode help
...
Examples:
  ...
  vcode run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | vcode run

Configuration:
  Resolution: flag > ./vcode.toml > ~/.vcode/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'vcode setup' to scaffold a config; see docs/SPEC.md.
`,
}
