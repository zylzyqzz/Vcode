...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to the Vcode home skills directory."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.vcode/skills/ (this workspace only); 'global' writes to the Vcode home skills directory (every project)."
	}
