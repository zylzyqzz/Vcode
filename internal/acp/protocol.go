...
// to drive Vcode. Many tools integrated with the v1 (main-branch) agent over
...
// older map shape that Vcode v1 clients used.
type MCPEnv map[string]string

// MCPHeaders accepts ACP's official HTTPHeader[] shape while still accepting
// the older map shape that Vcode v1 clients used. The official spec
...
// SessionListResult is the first and only page of sessions Vcode currently
