...
##      forces the user back to %LOCALAPPDATA%\Programs\Vcode even if they had
##      moved the install to a different drive (e.g. D:\Tools\Vcode); the silent
...
!define VCODE_DEFAULT_INSTALLDIR "$LOCALAPPDATA\Programs\${INFO_PRODUCTNAME}"
...
InstallDir "${VCODE_DEFAULT_INSTALLDIR}" # Per-user install location (no admin rights required).
...
!macro vcode.writeUninstaller
    ...
    # user back to %LOCALAPPDATA%\Programs\Vcode even if they had moved
    # the install to a different drive (e.g. D:\Tools\Vcode). The auto-
...
fallback:
   StrCpy $INSTDIR "${VCODE_DEFAULT_INSTALLDIR}"
done:
FunctionEnd
...
Section
    ...
    !insertmacro vcode.writeUninstaller
SectionEnd
...
Section "uninstall"
    ...
    !insertmacro vcode.deleteUninstaller
