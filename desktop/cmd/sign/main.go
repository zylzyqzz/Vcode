...
// by substring (file names embed the key, e.g. Vcode-darwin-arm64.zip), so the
...
// (vcode.key) and the public key (vcode.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "vcode.key")
	pubPath := filepath.Join(dir, "vcode.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Vcode release")
		out := f + ".minisig"
...
func genManifest(dir, version, tag string) error {
	...
	if repo == "" || repo == "esengine/vcode" {
		repo = "esengine/DeepSeek-Vcode"
	}
	m := update.Manifest{
		Version:      version,
