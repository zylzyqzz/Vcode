...
effort = "max"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "vcode.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
`
	if err := os.WriteFile(filepath.Join(root, "vcode.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestAutoResearchStatusSurfaceForActiveTab(t *testing.T) {
	...
	heartbeatPath := filepath.Join(root, ".vcode", "autoresearch", current.TaskID, "logs", "heartbeat.jsonl")
...
func TestAutoResearchFindingsAreLoadedOnDemand(t *testing.T) {
	...
	findingsPath := filepath.Join(root, ".vcode", "autoresearch", current.TaskID, "state", "findings.jsonl")
