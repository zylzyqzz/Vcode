...
A native desktop window around the Vcode Go kernel. The same
...
`desktop/` is its own Go module (`module Vcode/desktop`, `replace Vcode =>
...
`Vcode/` so it can still import the `Vcode/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/Vcode(.app/.exe)
```
...
`crash.Vcode.io` desktop release gateway) and shows an update banner when a
...
- **macOS** — open `Vcode-darwin-universal.dmg` and drag Vcode into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Vcode.app
  ```
...
```sh
minisign -Vm Vcode-darwin-arm64.zip \
...
- **Linux / WebKitGTK** is the one real pain point — rendering varies by distro &
  ...
  - **Wayland + NVIDIA**: On KDE Plasma Wayland with NVIDIA GPUs, WebKitGTK can
    ...
    Vcode automatically sets `__NV_DISABLE_EXPLICIT_SYNC=1` when it detects
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `Vcode_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  off.
...
The desktop app sends one anonymous ping per launch to `crash.Vcode.io`:
...
`agent.memory_compiler.enabled` setting with the CLI/TUI and `Vcode serve`;
CLI users can also run `/memory-v5 off|observe|compact|on|status` in a session
or `Vcode config memory-v5 off|observe|compact|on|status` from a shell.
