...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "vcode.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Vcode e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "vcode/internal/agent/foo.go" into
