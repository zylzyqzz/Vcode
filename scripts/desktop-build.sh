...
#   macOS:   Vcode-darwin-<arch>.zip                  (ditto archive; updater channel)
#            Vcode-darwin-universal.dmg               (drag-to-install; human download)
#   Windows: Vcode-windows-<arch>-installer.exe       (NSIS per-user installer; updater channel)
#            Vcode-windows-<arch>.zip                 (portable human download)
#   Linux:   Vcode-linux-<arch>.tar.gz                (bare binary; updater channel)
#            Vcode-linux-<arch>.deb                   (Debian/Ubuntu package; human download)
#
...
APPNAME="Vcode"            # wails.json productName -> Vcode.app
BINNAME="vcode-desktop"    # wails.json outputfilename -> linux binary name
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (vcode-desktop.app); repackage
	# it as Vcode.app for a clean user-facing name.
	staging=$(mktemp -d)
	app="$staging/${APPNAME}.app"
	cp -R "build/bin/vcode-desktop.app" "$app"
